...
var ChronoChat = function (screenName, chatRoom, hubPrefix, face, keyChain, certificateName) {
  ...
  if (this.screen_name == "" || this.chatroom == "") {
    ...
    this.sync = new ChronoSync2013(this.sendInterest.bind(this), this.initial.bind(this), this.chat_prefix,
    ...
    face.registerPrefix(this.chat_prefix, this.onInterest.bind(this),
      this.onRegisterFailed.bind(this));
  }
};
 ...
 */
ChronoChat.prototype.onInterest = function (prefix, interest, face, interestFilterId, filter) {
  var content = {};
  ...
  for (var i = this.msgcache.length - 1; i >= 0; i--) {
    if (this.msgcache[i].seqno == seq) {
      if (this.msgcache[i].msgtype != 'CHAT')
        content = new this.ChatMessage({
          from: this.screen_name,
          to: this.chatroom,
          type: this.msgcache[i].msgtype,
          timestamp: parseInt(this.msgcache[i].time / 1000)
        });
      else
        content = new this.ChatMessage({
          from: this.screen_name,
...
ChronoChat.prototype.initial = function () {
  ...
  if (this.roster.indexOf(this.usrname) == -1) {
    ...
    clearMembershipList();
    addMemberToMembershipList(this.screen_name);
    var d = new Date();
    joinedChatText(this.screen_name, this.chatroom, d.toLocaleTimeString());
  ...
  if (temp - content.timestamp * 1000 < 120000) {
    ...
    if (l == this.roster.length) {
      ...
      for (var i = 0; i < this.roster.length; i++) {
        ...
        addMemberToMembershipList(name_t);
      }
    }
    var timeout = new Interest(new Name("/timeout"));
...
ChronoChat.prototype.sendMessage = function () {
  ...
  if (chatmsg != "") {
    ...
    // Encode special html characters to avoid script injection.
    chatText(this.screen_name, tt, chatmsg, this.screen_name);
    var objDiv = document.getElementById("txt");
    objDiv.scrollTop = objDiv.scrollHeight;
  }
}
...
ChronoChat.prototype.leave = function () {
  ...
  // restart chronochat by refreshing the page
  leaveChat();
};
